...
* **macOS / Linux / OpenWrt**: 运行 `Client-Side/connect.sh`。
  ...
  * 选择 **5** 进入 **节点管理** (支持并发测速、按延迟自动排序、节点删除)。
...
如果你添加了多个 VLESS 节点，可以通过 `connect.sh` 的选项 **5** 进行管理：
1. **并发测速**: 同时对所有节点发起 TCP Ping，直观显示连接延迟（ms）。
2. **自动排序**: 一键测速并将最快的节点自动置顶，方便快速选择。
3. **节点维护**: 随时删除失效或不再需要的节点链接。
